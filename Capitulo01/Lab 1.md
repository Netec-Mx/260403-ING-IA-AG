...
pip install langchain langchain-openai python-dotenv jupyter notebook

# 6. Verificar la instalación
pip list | Select-String "langchain|openai|dotenv|jupyter|notebook"
```
...
New-Item -Path agente_basico.ipynb -ItemType File
```

2. Selecciona **File** en el menú superior, luego **Open Folder...**, busca **Documents**, luego selecciona la carpeta **lab01_agente_basico** y haz clic en el botón **Select Folder**. Abre el archivo `.gitignore` para proteger tus credenciales. Este paso es **obligatorio** antes de escribir cualquier API key. Escribe el siguiente contenido:
...
4. Abre el archivo **agente_basico.ipynb** y agrega el siguiente contenido como **+ Code**, sólo para verificar que funciona correctamente:

```
print("Notebook funcionando correctamente 🚀")
```
Guarda los cambios en todos los archivos.
...
Selecciona el navegador para acceder a Jupyter desde el localhost.

**Verificación:**

6. Agrega una nueva celda debajo de la que está predeterminada con el ícono **+** del costado superior. Y agrega el siguiente comando:

```
pip install python-dotenv
```
7. Agrega otra celda debajo, de la misma manera que contenga el siguiente comando:
...
print(api_key[:10] + "...")
```
8. Selecciona la opción **Run** del menú superior y haz clic en **Run All Cells**. Espera, y verifica que se impriman los 10 primeros caracteres de tu API Key al final de la última celda.

![Lab Image](../Images/Screenshot_1.png)

---

### Paso 2: Verificar la Instalación de LangChain y Cargar Variables de Entorno

1. En el notebook de Jupyter, modifica la primer celda de la siguiente manera:
...
```python
# Celda 2: Verificación de versiones
...
try:
    print(f"Python-dotenv versión: {version('python-dotenv')}")
except PackageNotFoundError:
    print("❌ python-dotenv no está instalado")

try:
    print(f"LangChain OpenAI versión: {version('langchain-openai')}")
except PackageNotFoundError:
...
3. Selecciona la última celda y edítala con el siguiente código para cargar las variables de entorno y verificar que la API key está disponible sin mostrarla completa:

```python
# Celda 3: Cargar variables de entorno de forma segura
from dotenv import load_dotenv

resultado = load_dotenv(".env", override=True)

print("Resultado load_dotenv:", resultado)

import os

print("API KEY:", os.getenv("AZURE_OPENAI_API_KEY")is not None)
print("ENDPOINT:", os.getenv("AZURE_OPENAI_ENDPOINT"))
print("DEPLOYMENT:", os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME"))
print("VERSION:", os.getenv("AZURE_OPENAI_API_VERSION"))
```

3. Ejecuta ambas celdas presionando `Shift+Enter` en cada una. Espera que termina cada una para ejecutar la siguiente. 

**Salida Esperada:**

**Celda 2**
```
LangChain versión: 1.3.4
Python-dotenv versión: 1.2.2
LangChain OpenAI versión: 1.2.2
✅ Verificación completada
```
**Celda 3**
```
Resultado load_dotenv: True
API KEY: True
...
- El modelo está configurado como `gpt-4o` o el que hayas definido
...
```python
# Celda 4: Importar componentes de LangChain
...
```python
# Celda 5: Inicializar el modelo de lenguaje (el "cerebro" del agente)
import os
...
print("✅ Modelo Azure OpenAI inicializado")
```
...
```python
# Celda 6: Diseñar el Prompt Template (instrucciones del agente)
...
```python
# Celda 7: Construir la cadena de ejecución (el "orquestador" básico)
...
```python
# Celda 8: Probar el chatbot con la primera pregunta
print("=" * 60)
...
```python
# Celda 9: Probar la segunda pregunta (observar falta de memoria)
print("=" * 60)
...
```
============================================================
CHATBOT BÁSICO - PRUEBA 2 (Sin memoria)
============================================================
👤 Usuario: ¿Puedes darme un ejemplo de lo que acabas de explicar?

🤖 Agente: ¡Por supuesto! Sin embargo, parece que no hay contexto previo en esta conversación para saber qué es lo que expliqué antes. Si me das más detalles o el tema específico al que te refieres, estaré encantado de proporcionarte un ejemplo práctico. Por favor, dime sobre qué necesitas el ejemplo. 😊

⚠️  OBSERVACIÓN: El agente no recuerda la pregunta anterior.
   Esto se debe a que no tiene memoria configurada.
============================================================
```
...
```python
# Celda 10: Importar componentes de memoria
...
```
```python
# Celda 11: Crear historial de conversación
...
```python
# Celda 12: Configurar la memoria conversacional
...
print("   Tipo de memoria: InMemoryChatMessageHistory")
print(f"   Mensajes almacenados: {len(memoria.messages)}")
print(f"   Historial inicial: {memoria.messages}")
```

3. Crea el prompt template que incluye el historial de conversación como variable:

```python
# Celda 13: Crear prompt template con soporte para historial
...
prompt_con_memoria = ChatPromptTemplate.from_messages([
    (
        "system",
        """Eres un asistente de IA amable y profesional especializado en
...
print("   Variables esperadas: history, input")
```

4. Construye el agente conversacional con memoria usando `ConversationChain`:

```python
# Celda 14: Construir el agente con memoria (LangChain 1.x)

from langchain_core.runnables.history import RunnableWithMessageHistory

# Cadena base
cadena_con_memoria_base = (
    prompt_con_memoria
    | llm
    | output_parser
)
...
print("   Componentes:")
print("   1. LLM (cerebro): AzureChatOpenAI")
print("   2. Memoria: InMemoryChatMessageHistory")
print("   3. Prompt: ChatPromptTemplate + MessagesPlaceholder")
print("   4. Orquestador: RunnableWithMessageHistory")
```

5. Prueba el agente con memoria usando una conversación de múltiples turnos:

```python
# Celda 15: Probar el agente con memoria - Turno 1
print("=" * 60)
...
respuesta_turno_2 = agente_con_memoria.invoke(
    {"input": turno_2},
    config=config
)
...
```python
# Celda 17: Turno 3 - Referencia a contexto previo
...
respuesta_turno_3 = agente_con_memoria.invoke(
    {"input": turno_3},
    config=config
)
...
```python
# Celda 18: Inspeccionar el contenido de la memoria
...
for i, mensaje in enumerate(memoria.messages, start=1):
    print(f"[{i}] {mensaje.type.upper()}:")
...
num_mensajes = len(memoria.messages)

num_humanos = sum(
    1 for msg in memoria.messages
    if msg.type == "human"
)
...
print(f"   Mensajes almacenados: {num_mensajes}")
print(f"   Mensajes de usuario: {num_humanos}")
print(f"   Mensajes del agente: {num_ai}")
...
```
============================================================
INSPECCIÓN DE LA MEMORIA DEL AGENTE
============================================================
...
1. Crea una nueva celda con el siguiente contenido, esta tendrá una función auxiliar para facilitar las comparaciones:

```python
# Celda 19: Función auxiliar para comparar prompts
...
```python
# Celda 20: Experimento 1 - Comparar nivel de especificidad del prompt
...
3. Documenta tus observaciones en una celda de texto (Markdown):

```python
# Documentar observaciones del experimento
print("=" * 70)
...
memoria_final = InMemoryChatMessageHistory()
...
prompt_final = ChatPromptTemplate.from_messages([
    (
        "system",
        """Eres TechBot, un asistente de IA especializado en tecnología y
...
# ─────────────────────────────────────────────
# COMPONENTE 5: Orquestador con Memoria
# ─────────────────────────────────────────────

techbot = RunnableWithMessageHistory(
    cadena_techbot,
    lambda session_id: memoria_final,
    input_messages_key="input",
    history_messages_key="history"
)
...
print("│ 💾 Memoria:     InMemoryChatMessageHistory             │")
print("│ 📋 Prompt:      ChatPromptTemplate                     │")
print("│ ⚙️ Orquestador: RunnableWithMessageHistory            │")
print("└─────────────────────────────────────────────────────────┘")
```
...
# Configuración de sesión (crear una sola vez)
...
def chat_con_techbot(mensaje):
    ...
    # Estadísticas de memoria
    num_mensajes = len(memoria_final.messages)
    turnos = num_mensajes // 2
...
# Estadísticas de la memoria moderna

total_mensajes = len(memoria_final.messages)
turnos_totales = total_mensajes // 2

print(f"✅ Turnos de conversación completados: {turnos_totales}")
print("✅ Memoria funcionando: El agente recordó el contexto en cada turno")
print("✅ Prompt personalizado: TechBot mantuvo su rol y características")
print("✅ Respuestas en español: Instrucción del prompt respetada")
...
print("  ⚙️ Orquestador → Gestionó historial, prompt y modelo")

print("\n📦 Detalle de memoria:")
print(f"   Mensajes almacenados: {total_mensajes}")
print(f"   Turnos completos: {turnos_totales}")
```
